...
<div class="bg-white dark:bg-gray-800 border bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 rounded-lg mb-8">
    <!-- Header -->
    <div class="flex justify-between items-center p-4 border-b border-gray-100 dark:border-gray-700">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <span class="font-bold text-gray-800 dark:text-gray-200 text-sm">{{ $post->user->name }}</span>
                <span class="text-xs text-gray-400 block">{{ $post->created_at->diffForHumans() }}</span>
            </div>
        </div>
    ...
    <div class="p-0">
        ...
        @endif
        <div class="px-4 pb-4">
            <p class="text-gray-800 dark:text-gray-200 text-base leading-relaxed">{{ $post->content }}</p>
        </div>
    </div>
    ...
    <div class="p-4 pt-0">
        <div class="flex items-center space-x-4 mb-2">
            <button onclick="toggleLike(this, {{ $post->id }}, {{ auth()->check() ? 'true' : 'false' }})" class="hover:opacity-75 transition group">
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 {{ auth()->check() && $post->likes->contains('user_id', auth()->id()) ? 'text-red-500 fill-current' : 'text-gray-800 dark:text-gray-200' }} transition duration-300 ease-in-out transform group-active:scale-125" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
            </button>

            <a href="{{ route('posts.show', $post) }}" class="hover:opacity-75 transition">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-gray-800 dark:text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
             ...
             <form action="{{ route('save.store') }}" method="POST" class="ml-auto">
                 ...
                 <button type="submit" class="hover:opacity-75 transition">
                     <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 {{ auth()->user()->savedPosts->contains($post->id) ? 'text-black dark:text-white fill-current' : 'text-gray-800 dark:text-gray-200' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
